...
![Photo by [veeterzy](https://unsplash.com/@veeterzy) on Unsplash](./veeterzy-tree.jpg)
...
React helps us to build UIs by creating reusable components like buttons or text fields, then composing a tree of the
...
During its life, a component instance goes through three different phases: mounting, updating and un-mounting. At the
...
function replaces the ***componentDidUpdate*** lifecycle methods of Class Components in a very convenient way.
...
state clicks. If all of those are the same as before we return ***false*** and stop the rendering process.
...
**So what?** You may argue… It's clear that writing Function Components using Hooks entails a lot of learning: new API,
